...
<div class="flex flex-col ${className}">
    <figure class="relative w-full bg-[#F5F7FA] rounded overflow-hidden mb-4 aspect-square">
        ...
        <c:if test="${requiresAdultVerification && !isAdultVerified}">
            <button 
                type="button" 
                class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center bg-[rgba(255,255,255,0.8)] rounded-full border border-[#CCD1D8] border-[0.6px] hover:bg-white transition-colors duration-200 ease-smooth focus:outline-none focus-visible:ring-2 focus-visible:ring-[#1B1E23] focus-visible:ring-offset-2 z-10"
                aria-label="성인인증">
    ...
    <div class="flex flex-col gap-2">
        ...
        <c:if test="${empty showPrice || showPrice != false}">
            <div class="flex flex-col">
                ...
                <c:if test="${discountPrice != null}">
                    <div class="flex gap-1 items-center">
                        ...
                        <c:if test="${discountPriceWon != null and discountPriceWon != ''}">
            ...
            <c:if test="${showFlags != false && !empty flags}">
